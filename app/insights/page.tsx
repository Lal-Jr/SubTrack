    ...
    return (
        <div>
            ...
            <div className="grid lg:grid-cols-2 gap-x-12 gap-y-10">
                <Section title="Weight" hint="Sized by yearly cost">
                    <div className="flex h-12 gap-[3px] rounded-lg overflow-hidden" role="img" aria-label="Yearly cost split by subscription">
                        {view.ranked.map((s) => (
                    ...
                    <div className="mt-5">
                        <BarList bars={false} currency={currency} unit="/yr" items={view.ranked.slice(0, 8).map((s) => ({ key: s.id, label: s.name, valueMinor: yearlyMinor(s), color: categoryColor(s.category || 'Other') }))} />
                    </div>
                </Section>
                ...
                <Section title="By category" hint="Monthly cost">
                    <BarList currency={currency} unit="/mo" items={view.categories.map((c) => ({ key: c.category, label: c.category, valueMinor: c.monthlyMinor, color: categoryColor(c.category), detail: `${(c.share * 100).toFixed(0)}% · ${c.count}` }))} />
                </Section>
                ...
                <Section title="Next 12 months" hint="From your real charge dates" className="lg:col-span-2">
